const RiskAssessmentFormData = {
  client_name: '',
  site_address: '',
  assessment_date: '',
  planned_review_date: '',
  submit_final: 0,
  form_status: '',

  vulnerability: '',
  review_frequency: '',
  dependent_on_homecare: 0,
  has_only_1_support_worker: 0,
  receives_personal_care: 0,
  reason_not_reliant: '',


  hearing_impairment: 0,
  hearing_hazards: '',
  hearing_management_plan: '',
  speech_impairment: 0,
  speech_hazards: '',
  speech_management_plan: '',


  oriented_in_time_place: 0,
  oriented_hazards: '',
  oriented_management_plan: '',
  accepts_direction: 0,
  direction_hazards: '',
  direction_management_plan: '',
  short_term_memory_issues: 0,
  memory_hazards: '',
  memory_management_plan: '',


  walk_unaided: 0,
  accessibility_required: 0,
  walk_hazards: '',
  walk_management_plan: '',

  manages_stairs: 0,
  stairs_hazards: '',
  stairs_management_plan: '',

  uses_walking_aid: 0,
  walking_aid_hazards: '',
  walking_aid_management_plan: '',

  uses_wheelchair: 0,
  wheelchair_hazards: '',
  wheelchair_management_plan: '',

  bed_transfer: 0,
  bed_transfer_hazards: '',
  bed_transfer_management_plan: '',

  vehicle_transfer: 0,
  vehicle_transfer_hazards: '',
  vehicle_transfer_management_plan: '',

  toilet_transfer: 0,
  toilet_transfer_hazards: '',
  toilet_transfer_management_plan: '',


  showering: 0,
  showering_hazards: '',
  showering_management_plan: '',

  meal: 0,
  meal_hazards: '',
  meal_management_plan: '',

  toileting: 0,
  toileting_hazards: '',
  toileting_management_plan: '',

  grooming: 0,
  grooming_hazards: '',
  grooming_management_plan: '',

  repositioning_bed: 0,
  repositioning_bed_hazards: '',
  repositioning_bed_management_plan: '',

  repositioning_chair: 0,
  repositioning_chair_hazards: '',
  repositioning_chair_management_plan: '',

  mouthcare: 0,
  mouthcare_hazards: '',
  mouthcare_management_plan: '',

  skin_care: 0,
  skin_care_hazards: '',
  skin_care_management_plan: '',



  physical_aggression: 0,
  physical_hazards: '',
  physical_management_plan: '',
  physical_bsp_plan: 0,

  verbal_aggression: '',
  verbal_hazards: '',
  verbal_management_plan: '',
  verbal_bsp_plan: 0,
  verbal_aggression_notes: '',

  client_aggression: 0,
  client_hazards: '',
  client_management_plan: '',
  client_bsp_plan: 0,

  self_harm: 0,
  self_harm_hazards: '',
  self_harm_management_plan: '',
  self_harm_bsp_plan: 0,

  drug_alcohol_use: 0,
  drug_alcohol_hazards: '',
  drug_alcohol_management_plan: '',
  drug_alcohol_bsp_plan: 0,

  sexual_abuse_history: 0,
  sexual_abuse_hazards: '',
  sexual_abuse_management_plan: '',
  sexual_abuse_bsp_plan: 0,

  emotional_manipulation: 0,
  emotional_hazards: '',
  emotional_management_plan: '',
  emotional_bsp_plan: 0,

  other_known_risks: 0,
  other_risks_hazards: '',
  other_risks_management_plan: '',
  other_risks_bsp_plan: 0,

  finance_management: '',
  finance_hazards: '',
  finance_management_plan: '',
  finance_bsp_plan: 0,
  finance_management_notes: '',



};
export default RiskAssessmentFormData;