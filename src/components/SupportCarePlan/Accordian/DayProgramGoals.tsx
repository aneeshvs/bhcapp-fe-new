...
interface PreviousDayProgramGoalsProps {
    ...
    setMyGoals?: React.Dispatch<React.SetStateAction<DayProgramGoal[]>>;
    uuid?: string | null;
}

export default function DayProgramGoals({ myGoals = [], setMyGoals, uuid }: PreviousDayProgramGoalsProps) {
    ...
    const addDayProgramGoals = () => {
        if (!setMyGoals) return;
        const newIndex = myGoals.length;
        setMyGoals([
            ...myGoals,
            {
                category: 'day_program',
    ...
    const removeDayProgramGoals = async (index: number) => {
        if (!setMyGoals) return;
    ...
    return (
        <div className="mb-4 mt-4 border border-gray-300 rounded shadow bg-white">
            <div className="flex justify-between items-center bg-gray-100 px-4 py-3">
                <h2 className="text-lg font-semibold text-heading">DAY PROGRAM GOALS</h2>
                {uuid && (
                    <button
                        type="button"
            ...
            <div className="p-4">
                {myGoals.length === 0 ? (
                    <p className="text-center text-gray-500 py-4">No Day Program goals added yet.</p>
                ) : (
                    myGoals.map((goal, index) => (
                        ...
                        >
                            ...
                            <button
                                type="button"
                                onClick={() => removeDayProgramGoals(index)}
